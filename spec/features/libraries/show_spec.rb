...
RSpec.describe 'library show page' do
  ...
  it 'displays three most popular authors by book number' do
    library = Library.create!(name: 'Moscow Public Library')
    author = Author.create!(name: 'Archie Long Fur', birth_year: 1990)
    author_2 = Author.create!(name: 'Luna Twoolie', birth_year: 1992)
    author_3 = Author.create!(name: 'Purple Eater', birth_year: 1990)
    author_4 = Author.create!(name: 'Orange Eater', birth_year: 1992)
    author_5 = Author.create!(name: 'Blue Eater', birth_year: 1992)
    book = library.books.create!(title: "A Dogs World", genre: "Kids")
    book_2 = library.books.create!(title: "A Cats World", genre: "Kids")
    book_3 = library.books.create!(title: "Catepillar", genre: "Kids")
    book_4 = library.books.create!(title: "Moth", genre: "Kids")
    author_book_1 = AuthorsBook.create!(book_id: book.id, author_id: author.id)
    author_book_1_2 = AuthorsBook.create!(book_id: book.id, author_id: author_2.id)
    author_book_1_3 = AuthorsBook.create!(book_id: book.id, author_id: author_3.id)
    author_book_1_4 = AuthorsBook.create!(book_id: book.id, author_id: author_4.id)
    author_book_1_5 = AuthorsBook.create!(book_id: book.id, author_id: author_5.id)
    author_book_2_1 = AuthorsBook.create!(book_id: book_2.id, author_id: author.id)
    author_book_2_4 = AuthorsBook.create!(book_id: book_2.id, author_id: author_4.id)
    author_book_2_5 = AuthorsBook.create!(book_id: book_2.id, author_id: author_5.id)
    author_book_3_1 = AuthorsBook.create!(book_id: book_3.id, author_id: author.id)
    author_book_5_4 = AuthorsBook.create!(book_id: book_3.id, author_id: author_4.id)

    visit "/libraries/#{library.id}"

    within "#popular-#{library.id}" do
      expect(page).to have_content("Archie Long Fur")
      expect(page).to have_content("Orange Eater")
      expect(page).to have_content("Blue Eater")
      expect(page).to_not have_content("Purple Eater")
      expect(page).to_not have_content("Luna Twoolie")
    end
  end
